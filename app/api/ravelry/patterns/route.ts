...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const ravelryUrl = new URL('https://api.ravelry.com/patterns/search.json')
    ...
    ravelryUrl.searchParams.set('sort', 'best')
    ...
    console.error('Error fetching patterns from Ravelry:', error)
    return NextResponse.json(
      { error: 'Failed to fetch patterns from Ravelry' },
      { status: 500 }
    )
  }
}
